...
<head>
    <title>Assign Machine to a Job Site</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>

<body>
    <div class="container mt-5">
        <h1>Assign Machine to a Job Site</h1>

        @if (isset($preselected_machine_id))
            <a href="{{ route('machines.show', $preselected_machine_id) }}" class="btn btn-secondary mb-3">Back to
                Machine</a>
        @endif
        @if (isset($preselected_jobsite_id))
            <a href="{{ route('jobsites.show', $preselected_jobsite_id) }}" class="btn btn-secondary mb-3">Back to
                Job site</a>
        @endif
        @if ($errors->any())
        ...
        <form action="{{ route('jobsitemachines.store') }}" method="POST">
            @csrf
            <table class="table">
                <tr>
                    <td>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jobsite_id" id="jobsite_id"
                                ...
                                @foreach ($jobsites as $id => $name)
                                    <option value="{{ $id }}"
                                        {{ old('jobsite_id', $preselected_jobsite_id ?? '') == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <tr>
                    ...
                    <td>
                        <div class="mb-3 km-reasons @if (old('status') != 'completed') d-none @endif">
